...
//
// In tunnel mode this means VXLAN encap happens prior to encryption and the
...
// vxlanTCPDumpFilters4 will generate the required TCPDump filters for leak
// detection.
//
// the TCPDump filter will:
//  1. detect UDP traffic
//  2. detect that the UDP dst port is the configured tunnel protocol port
//  3. seek into the VXLAN packet and ensure the IP header does not contain
//     [src: client, dst: server], this would be a leak.
func (s *podToPodEncryptionV2) vxlanTCPDumpFilters4(ctx context.Context) (clientFilter string, serverFilter string, err error) {
	if ctx.Err() != nil {
		return "", "", fmt.Errorf("Context already cancelled")
	}

	// Start at the UDP header (VXLAN) and index into IPHeader.Src and IPHeader.Dst
	// UDP(8)+VXLAN(8)+ETHER(14) = udp[30] + Offset to IPHeader.Src = udp[42]
	// UDP(8)+VXLAN(8)+ETHER(14) = udp[30] + Offset to IPHeader.Dst = udp[46]
...
func (s *podToPodEncryptionV2) resolveTCPDumpFilters4(ctx context.Context) (clientFilter string, serverFilter string, err error) {
	...
	// handle VXLAN tunneling mode.
	if s.tunnelMode.Enabled && s.tunnelMode.Mode == "vxlan" {
		return s.vxlanTCPDumpFilters4(ctx)
	}

	return s.nativeTCPDumpFilters4(ctx)
}

func (s *podToPodEncryptionV2) vxlanTCPDumpFilters6(ctx context.Context) (clientFilter string, serverFilter string, err error) {
	if ctx.Err() != nil {
		return "", "", fmt.Errorf("Context already cancelled")
	}

	// Start at the UDP header (VXLAN) and index into IP6Header.Src and IP6Header.Dst
	// UDP(8)+VXLAN(8)+ETHER(14) = udp[30] + Offset to IP6Header.Src = udp[38]
	// UDP(8)+VXLAN(8)+ETHER(14) = udp[30] + Offset to IP6Header.Dst = udp[54]
	//
...
func (s *podToPodEncryptionV2) resolveTCPDumpFilters6(ctx context.Context) (clientFilter string, serverFilter string, err error) {
	...
	if s.tunnelMode.Enabled && s.tunnelMode.Mode == "vxlan" {
		return s.vxlanTCPDumpFilters6(ctx)
	}
