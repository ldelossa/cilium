...
used_by=($(git grep -l "docker.io/library/golang:" images/*/Dockerfile))

for i in "${used_by[@]}" ; do
    # golang images with image digest
    sed -E "s|docker\.io/library/golang:[0-9]*\.[0-9]*(\.[0-9]*)?\@[a-z0-9:]+|${image}@${image_digest}|" "${i}" > "${i}.sedtmp" && mv "${i}.sedtmp" "${i}"
    # other golang images (e.g. golang-alpine images)
    sed -E "s|docker\.io/library/golang:[0-9]*\.[0-9]*(\.[0-9]*)?-\(.*\)@.*\@[a-z0-9:]+|${image_alpine}@${image_alpine_digest}|" "${i}" > "${i}.sedtmp" && mv "${i}.sedtmp" "${i}"
done
