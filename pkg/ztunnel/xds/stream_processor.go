...
func (sp *StreamProcessor) handleAuthorizationTypeURL(req *v3.DeltaDiscoveryRequest) error {
...
func (sp *StreamProcessor) handleDeltaDiscoveryReq(req *v3.DeltaDiscoveryRequest) error {
	...
	if req.ResponseNonce != "" {
		...
		if _, ok := sp.expectedNonce[req.ResponseNonce]; !ok {
			return fmt.Errorf("unexpected nonce %q received, expected one of %v", req.ResponseNonce)
		}
		...
		if req.ErrorDetail != nil {
			return fmt.Errorf("nack received for message: %s error: %s code: %d",
				req.ResponseNonce, req.ErrorDetail.Message, req.ErrorDetail.Code)
		}
		return nil
	}
...
func (sp *StreamProcessor) handleEPEvent(epEvent *EndpointEvent) error {
	...
	// drain any remaining endpoint events
	for ep := range sp.endpointRecv {
		...
		case epEvent := <-sp.endpointRecv:
			if err := sp.handleEPEvent(epEvent); err != nil {
				sp.log.Error("Failed to handle EndpointEvent", logfields.Error, err,
					logfields.EndpointID, epEvent.Endpoint.ID)
			}
		}
	}
}
