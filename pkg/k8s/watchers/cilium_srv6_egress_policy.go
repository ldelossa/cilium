...
func (k *K8sWatcher) ciliumSRv6EgressPolicyInit(ciliumClient *k8s.K8sCiliumClient) {
	_, egpController := informer.NewInformer(
		...
		cache.ResourceEventHandlerFuncs{
			AddFunc: func(obj interface{}) {
				...
				defer func() { k.K8sEventReceived(metricCSREP, metricCreate, valid, equal) }()
				if csrep := k8s.ObjToCSREP(obj); csrep != nil {
					valid = true
					err := k.addCiliumSRv6EgressPolicy(csrep)
					k.K8sEventProcessed(metricCSREP, metricCreate, err == nil)
				}
			},
			UpdateFunc: func(oldObj, newObj interface{}) {
				var valid, equal bool
				defer func() { k.K8sEventReceived(metricCSREP, metricUpdate, valid, equal) }()
				...
				k.K8sEventProcessed(metricCSREP, metricUpdate, addErr == nil)
			},
			DeleteFunc: func(obj interface{}) {
				var valid, equal bool
				defer func() { k.K8sEventReceived(metricCSREP, metricDelete, valid, equal) }()
				...
				k.K8sEventProcessed(metricCSREP, metricDelete, true)
			},
		},
	...
	k.blockWaitGroupToSyncResources(
		...
		k8sAPIGroupCiliumSRv6EgressPolicyV2,
	)

	go egpController.Run(wait.NeverStop)
	k.k8sAPIGroups.AddAPI(k8sAPIGroupCiliumSRv6EgressPolicyV2)
}
